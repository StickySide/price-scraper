...
class StandardParser(Parser):
    ...
    def get_items(
        ...
        try:
            ...
            item_cards = [
                item for item in soup.find_all("div", class_="item-cell")
                ]
        except ValueError:
        ...
        logger.info(f"[{name}] Parsing complete: "
                    f"{len(self.item_list)} items parsed")
...
class SeleniumParser(Parser):
    ...
    def get_items(
        ...
        def _get_price(item) -> int:
            price_block = item.find(
                'div', attrs={'data-testid': 'customer-price'}
                )
            price_unfiltered = price_block.find(
                'span', attrs={'aria-hidden': 'true'}).text
            price = round(float(''.join(letter for letter
                                        in price_unfiltered if
                                        letter not in ['$', ','])))
            return price
        ...
        def _get_link(item) -> str:
            return (
                'http://www.bestbuy.com/' + str(item.find('a').get('href'))
                )
